...
`,
  ...
  private buildReactExamFiles(version: ExamVersion) {
    ...
    const enunciado = [
      ...
      `> real (\`GET ${endpoint}\`) con \`fetch\` y lista los registros que devuelve. Los dos traen su archivo`,
      '> de test (Vitest + Testing Library) como guía de estilo.',
      '',
      '> Ejemplo de un registro de este recurso:',
      '>',
      '> ```json',
      ...
      `  - \`src/pages/${ClassName}FormPage.tsx\`: **YA IMPLEMENTADA** (formulario que crea registros nuevos`,
      `    con \`POST ${endpoint}\`). Sin test.`,
      ...
      '## Tu trabajo',
      '',
      '  Todas las piezas de abajo ya vienen completas: **no tenés que programarlas**, solo escribir los 2',
      `  archivos de test que faltan, siguiendo el mismo estilo que \`${ClassName}Card.test.tsx\` y`,
      `  \`${ClassName}ListaPage.test.tsx\`.`,
      ...
      '',
      `  1. **\`src/pages/${ClassName}FormPage.test.tsx\`** (mínimo 3 tests): al completar todos los campos de`,
      '     texto y enviar el formulario, se llama a `fetch` con `POST` y el endpoint correcto, y el registro',
      '     creado se agrega a la lista `data-testid="' + resource + '-creados"`; si falta algún campo de texto',
      '     obligatorio se muestra un `role="alert"` y NO se llama a `fetch`.',
      '  2. **`src/components/ContadorLimite.test.tsx`** (mínimo 4 tests): valor inicial correcto; el botón',
