...
`,
  ...
  private buildReactCrudExamFiles(version: ExamVersion) {
    ...
    const enunciado = [
      ...
      '  la tuya, con esta estructura:',
      '',
      '  - `src/models/todo.ts`: modelo de datos.',
      '  - `src/api/todoApi.ts`: funciones que hacen fetch (GET/POST/PATCH/DELETE).',
      '  - `src/App.tsx`: enrutador principal con el menú de navegación.',
      '  - `src/pages/TodoCrudPage.tsx`: CRUD completo (listar/crear/editar/eliminar).',
      '  - `src/pages/TodoStat1Page.tsx`: página de cálculo de ejemplo (promedio de duración).',
      '  - `src/pages/TodoStat2Page.tsx`: página de cálculo de ejemplo (buscador de tareas por texto).',
      '',
      '  **Tu trabajo es DUPLICAR y ADAPTAR estos mismos archivos** (podés renombrar archivos y',
      '  componentes con libertad — solo acordate de actualizar los `import` correspondientes) para armar:',
      '',
      '  1. **Pregunta 1** (CRUD): tu propio modelo (con los campos de tu variante), tus propias',
      '     funciones de API (apuntando al endpoint de arriba) y tu propia página de CRUD (lista + formulario).',
      '  2. **Pregunta 2** y **Pregunta 3**: tus propias páginas de cálculo, siguiendo el mismo patrón',
      '     (fetch → calcular → renderizar) que ves en `TodoStat1Page.tsx`/`TodoStat2Page.tsx`,',
      '     pero calculando lo que pide cada enunciado de abajo (no el mismo cálculo del ejemplo).',
      '',
      '  Actualizá `src/App.tsx` para agregar tus propias páginas al menú superior.',
      '',
      '## Preguntas',
      '',
      ...questions.map((q) => `### Pregunta ${q.order}: ${q.title} (${q.points} pts)\n\n${q.statement}\n`),
    ...
    const appTsx = [
      ...
      '          para armar el mismo patrón contra otra API (tu examen asignado).',
      '        </p>',
      '        <p>',
      '          <b>Instrucciones:</b> DUPLICÁ estas páginas y componentes, y adaptalas para tu variante.',
      '          Luego actualizá este archivo <code>App.tsx</code> para agregar los enlaces a tus propias pantallas en el menú.',
      '        </p>',
      '      </div>',
      '    </div>',
      '  );',
  ...
  private buildReactExamFiles(version: ExamVersion) {
    ...
    const enunciado = [
      ...
      '## Tu recurso de referencia (variante asignada)',
      '',
      `> \`src/components/${ClassName}Card.tsx\`, \`src/pages/${ClassName}ListaPage.tsx\` y`,
      `> \`src/pages/${ClassName}FormPage.tsx\` YA VIENEN RESUELTOS como ejemplo: un componente que muestra`,
      `> un registro de \`${resource}\` (campos: ${fields.map((f) => `\`${f.key}\` (${tsType(f.type)})`).join(', ')}),`,
      `> una página que consulta tu API real (\`GET ${endpoint}\`) con \`fetch\` y lista los registros que`,
      `> devuelve, y una página con un formulario que crea registros nuevos (\`POST ${endpoint}\`).`,
      '> Los tres traen sus archivos de test (Vitest + Testing Library) como guía. También hay un',
      '> componente chico de referencia con test, `src/components/Semaforo.tsx`.',
      '',
      '> Ejemplo de un registro de este recurso:',
      '>',
      '> ```json',
      ...
      '    (arriba) y las rutas de las 6 secciones del examen. No necesitás tocarlos.',
      `  - \`src/components/${ClassName}Card.tsx\`, \`src/pages/${ClassName}ListaPage.tsx\` y`,
      `    \`src/pages/${ClassName}FormPage.tsx\`: RESUELTOS (lista + formulario de referencia), con sus`,
      '    tests como guía de estilo.',
      '  - `src/components/Semaforo.tsx`: RESUELTO (componente chico de referencia), con su test como guía.',
      ...
      `  faltan, siguiendo el mismo estilo que \`${ClassName}Card.test.tsx\`, \`${ClassName}ListaPage.test.tsx\`,`,
      `  \`${ClassName}FormPage.test.tsx\` y \`Semaforo.test.tsx\`.`,
    ...
    const formFieldInputJsx = (f: { key: string; type: DartFieldType; label: string }) => {
      if (f.type === 'bool') {
        return [
          `        <div className="field toggle-row">`,
      ...
      const inputType = f.type === 'int' || f.type === 'double' ? 'number' : 'text';
      return [
        `        <div className="field">`,
        `          <label htmlFor="field-${f.key}">${f.label}</label>`,
        `          <input`,
    ...
    const formPageTsx = [
      ...
      '',
      `const EMPTY_FORM = {`,
      ...fields.map((f) => `  ${f.key}: ${f.type === 'bool' ? 'false as boolean' : "''"},`),
      `};`,
      '',
      `export function ${ClassName}FormPage() {`,
      `  const [form, setForm] = useState(EMPTY_FORM);`,
      `  const [error, setError] = useState('');`,
      `  const [creados, setCreados] = useState<${ClassName}Item[]>([]);`,
      '',
      ...
      `  async function handleSubmit(e: FormEvent) {`,
      `    e.preventDefault();`,
      `    const textKeys: (keyof typeof EMPTY_FORM)[] = [${textFields.map((f) => `'${f.key}'`).join(', ')}];`,
      `    const falta = textKeys.some((k) => String(form[k]).trim() === '');`,
      `    if (falta) {`,
      ...
      `    const payload = {`,
      ...fields.map((f) => {
        if (f.type === 'bool') return `      ${f.key}: Boolean(form.${f.key}),`;
        if (f.type === 'int' || f.type === 'double') return `      ${f.key}: Number(form.${f.key}),`;
      ...
      `    setCreados((prev) => [...prev, nuevo]);`,
      `    setForm(EMPTY_FORM);`,
      `  }`,
      '',
      `  return (`,
      `    <div className="page">`,
      `      <h2 className="page-title">Crear ${resource}</h2>`,
      `      <form className="card" onSubmit={handleSubmit}>`,
      ...fields.map((f) => formFieldInputJsx(f)),
      `        <button className="btn" type="submit">Crear</button>`,
      `        {error && <p role="alert" className="alert">{error}</p>}`,
      `      </form>`,
      `      <ul className="list" data-testid="${resource}-creados">`,
      `        {creados.map((item) => (`,
      `          <li key={item.id}>${fieldValueJsx(f0)}</li>`,
      `        ))}`,
      `      </ul>`,
      `    </div>`,
      `  );`,
    ...
    const formPageTest = [
      ...
      `import { ${ClassName}FormPage } from './${ClassName}FormPage';`,
      '',
      `const nuevoCreado = ${JSON.stringify({ id: '99', ...seeds[0] })};`,
      '',
      `beforeEach(() => {`,
      `  global.fetch = vi.fn().mockResolvedValue({`,
      `    json: () => Promise.resolve(nuevoCreado),`,
      `  } as Response);`,
      `});`,
      '',
      `describe('${ClassName}FormPage', () => {`,
      `  it('debe enviar el formulario a la API y agregar el resultado a la lista de creados', async () => {`,
      `    render(<${ClassName}FormPage />);`,
      ...fields
        .filter((f) => f.type !== 'bool')
        .map((f) => `    fireEvent.change(screen.getByLabelText('${f.label}'), { target: { value: '${String((seeds[0] as any)[f.key] ?? 'x')}' } });`),
      `    fireEvent.click(screen.getByRole('button', { name: 'Crear' }));`,
      `    await waitFor(() => {`,
      `      expect(screen.getByTestId('${resource}-creados').children.length).toBe(1);`,
      `    });`,
      `    expect(global.fetch).toHaveBeenCalledWith('${endpoint}', expect.objectContaining({ method: 'POST' }));`,
      `  });`,
      '',
      `  it('debe mostrar un error y no llamar a la API si falta un campo de texto obligatorio', () => {`,
      `    render(<${ClassName}FormPage />);`,
      `    fireEvent.click(screen.getByRole('button', { name: 'Crear' }));`,
    ...
    const semaforoTsx = [
      ...
      '',
      `const ESTADOS = ['rojo', 'amarillo', 'verde'] as const;`,
      '',
      `/** Componente de referencia: cicla entre 3 estados al hacer click. */`,
      ...
      '',
      `  return (`,
      `    <div className="card semaforo-row">`,
      `      <span data-testid="semaforo-estado">{estado}</span>`,
      `      <button className="btn" onClick={siguiente}>Cambiar</button>`,
      `    </div>`,
      `  );`,
      `}`,
      '',
    ...
    const semaforoTest = [
      ...
      '',
      `describe('Semaforo', () => {`,
      `  it('inicia en rojo y cicla rojo -> amarillo -> verde -> rojo al hacer click', () => {`,
      `    render(<Semaforo />);`,
      `    const estado = screen.getByTestId('semaforo-estado');`,
      `    const boton = screen.getByRole('button', { name: 'Cambiar' });`,
      `    expect(estado).toHaveTextContent('rojo');`,
      `    fireEvent.click(boton);`,
      `    expect(estado).toHaveTextContent('amarillo');`,
      `    fireEvent.click(boton);`,
      `    expect(estado).toHaveTextContent('verde');`,
      `    fireEvent.click(boton);`,
      `    expect(estado).toHaveTextContent('rojo');`,
      `  });`,
      `});`,
    ...
    const stylesCss = [
      ...
      '',
      `.semaforo-row { display: flex; align-items: center; gap: 14px; }`,
      '',
    ].join('\n');
    ...
    const appTsx = [
      ...
      `import { ${ClassName}FormPage } from './pages/${ClassName}FormPage';`,
      ...
      `      <h3>Referencia (ya resuelto, con test de ejemplo)</h3>`,
      `      <Semaforo />`,
      `      <h3>Contador con límites (sin test todavía)</h3>`,
      `      <ContadorLimite />`,
      `      <h3>Toggle de habilitación (sin test todavía)</h3>`,
      `      <ToggleControl />`,
      `    </div>`,
      `  );`,
      `}`,
    ...
    return [
      ...
      { name: 'Semaforo.tsx', path: '/src/components/Semaforo.tsx', content: semaforoTsx, is_folder: false },
      { name: 'Semaforo.test.tsx', path: '/src/components/Semaforo.test.tsx', content: semaforoTest, is_folder: false },
      ...
      { name: `${ClassName}FormPage.tsx`, path: `/src/pages/${ClassName}FormPage.tsx`, content: formPageTsx, is_folder: false },
      { name: `${ClassName}FormPage.test.tsx`, path: `/src/pages/${ClassName}FormPage.test.tsx`, content: formPageTest, is_folder: false },
  ...
  private buildReactNativeExamFiles(version: ExamVersion) {
    ...
    const enunciado = [
      ...
      '## Punto de partida: app "ToDo" completa y funcional',
      '',
      '  El preview de React Native (Expo Snack) solo lee un único archivo, `App.tsx` — por eso',
      '  todo el proyecto vive ahí adentro, organizado por secciones con comentarios.',
      '',
      '  La sección **"Referencia (Tareas)"** ya viene RESUELTA: un CRUD completo (tareas con',
      '  `nombre` (string), `hecho` (bool), `duracion` (number) y `presupuesto` (number)) contra',
      `  otra API distinta a la tuya (\`${todoEndpoint}\`), más 2 pantallas de cálculo de ejemplo`,
      '  (promedio de duración / búsqueda por nombre). Podés navegar a esa sección desde el menú',
      '  ("Referencia") para ver cómo funciona antes de escribir la tuya.',
      '',
      '  **Tu trabajo es DUPLICAR y ADAPTAR ese mismo patrón** (fetch → estado → render) dentro',
      '  de las 3 funciones marcadas con `// TODO` más abajo en este mismo archivo (buscalas por',
      `  el nombre de la función: \`Mi${ClassName}Screen\`, \`Pregunta2Screen\`, \`Pregunta3Screen\`).`,
      '  No hace falta que borres la sección de referencia: podés dejarla como guía y solo',
      '  completar las 3 pantallas propias.',
      '',
      '  1. **Pregunta 1** (`Mi' + ClassName + 'Screen`): CRUD (listar/crear/editar/eliminar)',
      '     contra el endpoint de arriba, con los campos de tu variante.',
      '  2. **Pregunta 2** (`Pregunta2Screen`) y **Pregunta 3** (`Pregunta3Screen`): tus propias',
      '     pantallas de cálculo, siguiendo el mismo patrón que `PromedioScreen`/`BusquedaScreen`',
      '     de la referencia, pero calculando lo que pide cada enunciado de abajo (no el mismo',
      '     cálculo del ejemplo).',
      '',
    ...
    const appTsx = [
      ...
      "} from 'react-native';",
      '',
      '// ── Referencia (resuelta): modelo, API y pantallas de la app "Tareas" ──────────────',
      '',
      'interface Todo {',
      '  id: string;',
      '  nombre: string;',
      ...
      '                </TouchableOpacity>',
      '                <TouchableOpacity style={styles.btnSmall} onPress={() => deleteTodo(item.id).then(load)}>',
      ...
      '}',
      '',
      '/** Ejemplo de pantalla de cálculo (referencia): promedio de duración de las tareas. */',
      'function ReferenciaPromedioScreen() {',
      '  const [todos, setTodos] = useState<Todo[]>([]);',
      '  const [loading, setLoading] = useState(true);',
      '',
      '  useEffect(() => {',
      ...
      '}',
      '',
      '/** Ejemplo de pantalla de cálculo (referencia): búsqueda de tareas por nombre. */',
      'function ReferenciaBusquedaScreen() {',
      '  const [todos, setTodos] = useState<Todo[]>([]);',
      "  const [filtro, setFiltro] = useState('');",
      '  const [loading, setLoading] = useState(true);',
      '',
      ...
      '}',
      '',
      `// ── Tu trabajo: completá estas 3 pantallas duplicando el patrón de arriba ─────────`,
      `// Endpoint de tu variante (\`${resource}\`): ${endpoint}`,
      '',
      `/** Pregunta 1: CRUD completo de tu variante (\`${resource}\`) contra tu propia API. */`,
      `function Mi${ClassName}Screen() {`,
      '  // TODO: duplicá el patrón de `ReferenciaScreen`, pero apuntando a:',
      `  //   const API_URL = '${endpoint}';`,
      `  // y con los campos de tu variante: ${fields.map((f) => `\`${f.key}\` (${tsType(f.type)})`).join(', ')}.`,
      '  return (',
      '    <View style={styles.card}>',
      `      <Text style={styles.title}>${ClassName} (Pregunta 1) — no implementado</Text>`,
      '      <Text>Completá esta pantalla siguiendo el patrón de "Referencia".</Text>',
      '    </View>',
      '  );',
      '}',
      '',
      '/** Pregunta 2: tu propia pantalla de cálculo. */',
      'function Pregunta2Screen() {',
      '  // TODO: duplicá el patrón de `ReferenciaPromedioScreen`, calculando lo que pide',
      '  // el enunciado de la Pregunta 2 (ver ENUNCIADO.md) a partir de los datos de tu API.',
      '  return (',
      '    <View style={styles.card}>',
      ...
      '}',
      '',
      '/** Pregunta 3: tu propia pantalla de cálculo. */',
      'function Pregunta3Screen() {',
      '  // TODO: duplicá el patrón de `ReferenciaBusquedaScreen`, calculando lo que pide',
      '  // el enunciado de la Pregunta 3 (ver ENUNCIADO.md) a partir de los datos de tu API.',
      '  return (',
      '    <View style={styles.card}>',
      ...
      '}',
      '',
      "type Screen = 'home' | 'referencia' | 'ref-promedio' | 'ref-busqueda' | 'mi-crud' | 'p2' | 'p3';",
      '',
      ...
      `    { key: 'mi-crud', label: '${ClassName} (P1)' },`,
      ...
      '  switch (screen) {',
      "    case 'referencia':",
      '      content = <ReferenciaScreen />;',
      '      break;',
      "    case 'ref-promedio':",
      '      content = <ReferenciaPromedioScreen />;',
      '      break;',
      "    case 'ref-busqueda':",
      '      content = <ReferenciaBusquedaScreen />;',
      '      break;',
      "    case 'mi-crud':",
      `      content = <Mi${ClassName}Screen />;`,
      '      break;',
      "    case 'p2':",
      '      content = <Pregunta2Screen />;',
      '      break;',
      "    case 'p3':",
      '      content = <Pregunta3Screen />;',
      '      break;',
      '    default:',
      '      content = <HomeScreen />;',
      '  }',
