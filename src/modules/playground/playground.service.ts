...
`,
  ...
  private buildReactNativeExamFiles(version: ExamVersion) {
    ...
    const enunciado = [
      ...
      `  - \`src/api/${resource}Api.ts\`: API de tu variante (\`${ClassName}\`) — a completar.`,
      ...
      '  (`src/pages/ReferenciaPromedio.tsx` / `src/pages/ReferenciaBusqueda.tsx`). Podés navegar',
      ...
      '  de las 3 páginas marcadas con `// TODO` en `src/pages/`, apoyándote en',
      `  \`src/api/${resource}Api.ts\` (también marcado con \`// TODO\`):`,
      ...
      '',
      `  1. **Pregunta 1** (\`src/pages/Mi${ClassName}.tsx\` + \`src/api/${resource}Api.ts\`): CRUD`,
      '     (listar/crear/editar/eliminar) contra el endpoint de arriba, con los campos de tu variante.',
    ...
    const resourceApiTs = [
      ...
      '}',
      '',
      `const ${resource.toUpperCase()}_API_URL = '${endpoint}';`,
      '',
      `// TODO: duplicá el patrón de `,
      "// 'src/api/referenciaApi.ts', implementando estas 4 funciones contra",
      `// ${resource.toUpperCase()}_API_URL (definida arriba).`,
      '',
      `export async function fetch${ClassName}s(): Promise<${ClassName}[]> {`,
      `  throw new Error('TODO: implementar fetch${ClassName}s');`,
      '}',
      '',
      `export async function create${ClassName}(data: Omit<${ClassName}, 'id'>): Promise<${ClassName}> {`,
      `  throw new Error('TODO: implementar create${ClassName}');`,
      '}',
      '',
      `export async function update${ClassName}(id: string, data: Omit<${ClassName}, 'id'>): Promise<${ClassName}> {`,
      `  throw new Error('TODO: implementar update${ClassName}');`,
      '}',
      '',
      `export async function delete${ClassName}(id: string): Promise<void> {`,
      `  throw new Error('TODO: implementar delete${ClassName}');`,
      '}',
      '',
    ].join('\n');
    ...
    const miScreenTsx = [
      "import { View, Text } from 'react-native';",
      `import { ${ClassName}, fetch${ClassName}s, create${ClassName}, update${ClassName}, delete${ClassName} } from '../api/${resource}Api';`,
      "import { styles } from '../styles';",
      '',
      `/** Pregunta 1: CRUD completo de tu variante (\`${resource}\`) contra tu propia API. */`,
      `export function Mi${ClassName}Page() {`,
      '  // TODO: implementá primero las funciones de `src/api/' + resource + 'Api.ts`, y luego',
      '  // duplicá acá el patrón de `src/pages/Referencia.tsx` (fetch en useEffect, estado con',
      `  // useState<${ClassName}[]>, formulario y lista) usando las funciones importadas arriba.`,
      '  return (',
      '    <View style={styles.card}>',
    ...
    const pregunta2Tsx = [
      "import { View, Text } from 'react-native';",
      `import { fetch${ClassName}s } from '../api/${resource}Api';`,
      "import { styles } from '../styles';",
      '',
      '/** Pregunta 2: tu propia página de cálculo. */',
      'export function Pregunta2Page() {',
      '  // TODO: duplicá el patrón de `src/pages/ReferenciaPromedio.tsx`, pero llamando a',
      `  // fetch${ClassName}s() y calculando lo que pide el enunciado de la Pregunta 2`,
      '  // (ver ENUNCIADO.md) a partir de los datos de tu API.',
      '  return (',
      '    <View style={styles.card}>',
    ...
    const pregunta3Tsx = [
      ...
      '  // TODO: duplicá el patrón de `src/pages/ReferenciaBusqueda.tsx`, pero llamando a',
      `  // fetch${ClassName}s() y calculando lo que pide el enunciado de la Pregunta 3`,
      '  // (ver ENUNCIADO.md) a partir de los datos de tu API.',
      '  return (',
      '    <View style={styles.card}>',
    ...
    const appRouterTsx = [
      ...
      `import { Mi${ClassName}Page } from '../pages/Mi${ClassName}';`,
      "import { Pregunta2Page } from '../pages/Pregunta2';",
      "import { Pregunta3Page } from '../pages/Pregunta3';",
      "import { styles } from '../styles';",
      '',
      "export type Screen = 'home' | 'referencia' | 'ref-promedio' | 'ref-busqueda' | 'mi-crud' | 'p2' | 'p3';",
      '',
    ...
    return [
      ...
      { name: `${resource}Api.ts`, path: `/src/api/${resource}Api.ts`, content: resourceApiTs, is_folder: false },
      { name: 'components', path: '/src/components', content: '', is_folder: true },
      ...
      { name: `Mi${ClassName}.tsx`, path: `/src/pages/Mi${ClassName}.tsx`, content: miScreenTsx, is_folder: false },
