...
`,
  ...
  private buildReactNativeExamFiles(version: ExamVersion) {
    ...
    const enunciado = [
      ...
      '## Punto de partida: app "ToDo" completa y funcional',
      '',
      '  El proyecto está organizado en varios archivos cortos:',
      '',
      '  - `App.tsx`: menú de navegación y switch entre pantallas.',
      '  - `src/api.ts`: API de referencia (`Todo`, fetch/create/update/delete).',
      '  - `src/styles.ts`: estilos compartidos por todas las pantallas.',
      '  - `src/screens/*.tsx`: una pantalla por archivo.',
      '',
      '  El preview (Expo Snack) carga todos estos archivos, no solo `App.tsx`.',
      '',
      '  La pantalla **`src/screens/Referencia.tsx`** ya viene RESUELTA: un CRUD completo (tareas con',
      ...
      '  (`src/screens/ReferenciaPromedio.tsx` / `src/screens/ReferenciaBusqueda.tsx`). Podés navegar',
      '  a esa sección desde el menú ("Referencia") para ver cómo funciona antes de escribir la tuya.',
      '',
      '  **Tu trabajo es DUPLICAR y ADAPTAR ese mismo patrón** (fetch → estado → render) dentro',
      '  de las 3 pantallas marcadas con `// TODO` en `src/screens/`:',
      `  \`Mi${ClassName}.tsx\`, \`Pregunta2.tsx\`, \`Pregunta3.tsx\`.`,
      '  No hace falta que borres la sección de referencia: podés dejarla como guía y solo',
      '  completar las 3 pantallas propias.',
      '',
      '  1. **Pregunta 1** (`src/screens/Mi' + ClassName + '.tsx`): CRUD (listar/crear/editar/eliminar)',
      '     contra el endpoint de arriba, con los campos de tu variante.',
      '  2. **Pregunta 2** (`src/screens/Pregunta2.tsx`) y **Pregunta 3** (`src/screens/Pregunta3.tsx`):',
      '     tus propias pantallas de cálculo, siguiendo el mismo patrón que `ReferenciaPromedio.tsx`/',
    ...
    const referenciaPromedioTsx = [
      ...
      'export function ReferenciaPromedioScreen() {',
      '  const [todos, setTodos] = useState<Todo[]>([]);',
      '  const [loading, setLoading] = useState(true);',
      '',
      '  useEffect(() => {',
    ...
    const referenciaBusquedaTsx = [
      ...
      'export function ReferenciaBusquedaScreen() {',
      '  const [todos, setTodos] = useState<Todo[]>([]);',
      "  const [filtro, setFiltro] = useState('');",
      '  const [loading, setLoading] = useState(true);',
      '',
    ...
    const miScreenTsx = [
      ...
      `export function Mi${ClassName}Screen() {`,
      '  // TODO: duplicá el patrón de `src/screens/Referencia.tsx`, pero apuntando a:',
      `  //   const API_URL = '${endpoint}';`,
    ...
    const pregunta2Tsx = [
      ...
      'export function Pregunta2Screen() {',
      '  // TODO: duplicá el patrón de `src/screens/ReferenciaPromedio.tsx`, calculando lo que pide',
    ...
    const pregunta3Tsx = [
      ...
      'export function Pregunta3Screen() {',
      '  // TODO: duplicá el patrón de `src/screens/ReferenciaBusqueda.tsx`, calculando lo que pide',
    ...
    const appTsx = [
      ...
      "import { ReferenciaScreen } from './src/screens/Referencia';",
      "import { ReferenciaPromedioScreen } from './src/screens/ReferenciaPromedio';",
      "import { ReferenciaBusquedaScreen } from './src/screens/ReferenciaBusqueda';",
      `import { Mi${ClassName}Screen } from './src/screens/Mi${ClassName}';`,
      "import { Pregunta2Screen } from './src/screens/Pregunta2';",
      "import { Pregunta3Screen } from './src/screens/Pregunta3';",
    ...
    return [
      ...
      { name: 'src', path: '/src', content: '', is_folder: true },
      { name: 'api.ts', path: '/src/api.ts', content: apiTs, is_folder: false },
      { name: 'styles.ts', path: '/src/styles.ts', content: stylesTs, is_folder: false },
      { name: 'screens', path: '/src/screens', content: '', is_folder: true },
      { name: 'Referencia.tsx', path: '/src/screens/Referencia.tsx', content: referenciaTsx, is_folder: false },
      { name: 'ReferenciaPromedio.tsx', path: '/src/screens/ReferenciaPromedio.tsx', content: referenciaPromedioTsx, is_folder: false },
      { name: 'ReferenciaBusqueda.tsx', path: '/src/screens/ReferenciaBusqueda.tsx', content: referenciaBusquedaTsx, is_folder: false },
      { name: `Mi${ClassName}.tsx`, path: `/src/screens/Mi${ClassName}.tsx`, content: miScreenTsx, is_folder: false },
      { name: 'Pregunta2.tsx', path: '/src/screens/Pregunta2.tsx', content: pregunta2Tsx, is_folder: false },
      { name: 'Pregunta3.tsx', path: '/src/screens/Pregunta3.tsx', content: pregunta3Tsx, is_folder: false },
    ];
  }
