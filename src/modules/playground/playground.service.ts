...
export class PlaygroundService {
  ...
  private buildExamVersionFiles(version: ExamVersion, fileMode: 'single' | 'perQuestion', language?: string) {
    if (version.examTemplate?.name?.includes('React — CRUD y pantallas de lógica')) return this.buildReactCrudExamFiles(version);
...
`,
  ...
  private buildReactExamFiles(version: ExamVersion) {
    ...
    const enunciado = [
      ...
      '> con sus dos archivos de test (Vitest + Testing Library) como guía.',
      '',
      '> Ejemplo de un registro de este recurso:',
      '>',
      '> ```json',
      ...
      '    (arriba) y las rutas de las 5 secciones del examen. No necesitás tocarlos.',
