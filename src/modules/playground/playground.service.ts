...
`,
  ...
  private buildReactNativeExamFiles(version: ExamVersion) {
    ...
    const enunciado = [
      ...
      '  dentro de las 3 páginas marcadas con `// TODO` en `src/pages/`, apoyándote en',
      ...
      '',
      `  1. **Pregunta 1** (\`src/pages/Pregunta1.tsx\` + \`src/api/${resource}Api.ts\`): CRUD`,
      '     (listar/crear/editar/eliminar) contra el endpoint de arriba, con los campos de tu variante.',
      '     Te recomendamos crear tu propio formulario y lista como componentes separados en',
      '     `src/components/` (igual que `TareaForm.tsx`/`TareaList.tsx`), en vez de escribir todo',
      '     junto dentro de la página.',
      ...
      '',
      '  **Importante — renombrá Pregunta 1 según tu variante:** el archivo `Pregunta1.tsx`',
      '  (función `Pregunta1Page`, ítem de menú "Pregunta 1") viene con un nombre genérico para',
      '  que sigas el mismo patrón que Pregunta 2 y Pregunta 3, pero DEBÉS renombrarlo vos de',
      '  acuerdo al recurso de tu variante y a la regla de negocio del enunciado (por ejemplo, si',
      '  tu variante es de vehículos: archivo `MiAutos.tsx`, función `MiAutosPage`, etiqueta de',
      '  menú "Autos"). Actualizá también el import y el `case` correspondiente en',
    ...
    const tareaListTsx = [
      ...
      '          </Text>',
      '          <View style={styles.row}>',
      '            <TouchableOpacity style={styles.btnSmall} onPress={() => onEdit(item)}>',
      '              <Text style={styles.btnText}>Editar</Text>',
      '            </TouchableOpacity>',
      '            <TouchableOpacity style={styles.btnSmall} onPress={() => onDelete(item.id)}>',
    ...
    const pregunta1Tsx = [
      ...
      '// TODO: renombrá este archivo y esta función de acuerdo al recurso de tu variante',
      '// (por ejemplo, `Pregunta1.tsx`/`Pregunta1Page` -> `MiAutos.tsx`/`MiAutosPage`), y',
      '// actualizá el import + case en `src/router/AppRouter.tsx` y la etiqueta en',
      '// `src/components/Menu.tsx` para que coincidan.',
      ...
      `  // useState<${ClassName}[]>, y un formulario + una lista separados en sus propios`,
