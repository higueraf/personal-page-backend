"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.VideoStreamController = void 0;
const common_1 = require("@nestjs/common");
const fs_1 = require("fs");
const video_courses_service_1 = require("./video-courses.service");
let VideoStreamController = class VideoStreamController {
    constructor(service) {
        this.service = service;
    }
    async stream(lessonId, res) {
        const videoFile = await this.service.getVideoFileByLessonId(lessonId);
        if (!videoFile)
            throw new common_1.NotFoundException('No hay archivo de video');
        if (/^https?:\/\//i.test(videoFile)) {
            return res.redirect(videoFile);
        }
        if (!(0, fs_1.existsSync)(videoFile)) {
            throw new common_1.NotFoundException('Archivo no encontrado');
        }
        const stat = (0, fs_1.statSync)(videoFile);
        res.setHeader('Content-Type', 'video/mp4');
        res.setHeader('Content-Length', stat.size);
        return (0, fs_1.createReadStream)(videoFile).pipe(res);
    }
};
exports.VideoStreamController = VideoStreamController;
__decorate([
    (0, common_1.Get)(':lessonId'),
    __param(0, (0, common_1.Param)('lessonId')),
    __param(1, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], VideoStreamController.prototype, "stream", null);
exports.VideoStreamController = VideoStreamController = __decorate([
    (0, common_1.Controller)('video-stream'),
    __metadata("design:paramtypes", [video_courses_service_1.VideoCoursesService])
], VideoStreamController);
//# sourceMappingURL=video-stream.controller.js.map